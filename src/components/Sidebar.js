...
let Sidebar = () => {
    ...
    return (
        <div className="sticky top-0 left-0 w-full px-6 bg-black text-white max-h-screen overflow-y-auto z-50 md:h-full md:z-0">
            <div className="md:hidden w-full py-4">
                <button onClick={() => setMenuOpen(oldVal => !oldVal)}>{menuOpen ? 'Close' : 'Open'}</button>
            </div>

            <div className={`py-4 ${menuOpen ? 'block' : 'hidden'} md:block`}>  
